...
**Add a rubric-generation step before scoring (universal best practice).** Instructing the judge model to define its own scoring criteria before issuing a verdict is the single highest-return structural change for any judge prompt, across all model families — not just Gemini. Evidence: Sage benchmark (+16.1% IPI across tested judges, arxiv 2512.16041); Rethinking Rubric Generation (GPT-4o +17.7 pts on JudgeBench, Llama-405B +7.4 pts, arxiv 2602.05125); RubricBench confirms the "Rubric Gap" (~27 pts, self-generated vs. human rubrics) is consistent across Gemini, GPT, and DeepSeek (arxiv 2603.01562).

Embed this two-step block in every judge prompt:
...
<scoring>
Apply the rubric you just wrote. Rate on a 1–4 scale:
1 = Fails entirely, 2 = Partial, 3 = Mostly meets criterion, 4 = Fully meets.
</scoring>
```

**Rubric source hierarchy (when resources allow):** Human-written rubrics outperform self-generated by ~27 points (RubricBench). Cross-model rubric generation — using a different frontier model to draft the rubric — can outperform pure self-generation (RRD paper: GPT-4o judging with Gemini-generated rubrics outperforms GPT-4o self-generation). Use self-generated rubrics (embedded instruction) as the practical default; escalate to cross-model or human rubrics for highest-stakes scoring.
...
12. **Judge prompt: self-generated rubric (conditional — highest single-change ROI for judge prompts, universal across model families).** If the prompt's output is a quality judgment, does it include a `<rubric_generation>` block instructing the judge model to define scoring criteria before issuing a verdict? This is universal: GPT-4o +17.7 pts on JudgeBench, Llama-405B +7.4 pts, Sage aggregate +16.1% IPI (arxiv 2602.05125, 2512.16041). Embedded self-generation (same model, at inference time) is the practical default and meaningfully better than no rubric. For highest-stakes scoring, escalate to cross-model rubric generation (a different frontier model drafts rubric criteria) or human-written rubrics, which outperform self-generated by ~27 pts (RubricBench, arxiv 2603.01562). Also check: small integer rating scale (1–4) with indicative descriptions per level, and a `<reasoning>` field before the verdict. N/A for non-judge prompts.
