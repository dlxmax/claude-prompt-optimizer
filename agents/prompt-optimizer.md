...
12. **Sampling and determinism for judges (conditional).** Applies to high-stakes judge prompts. Check for: (a) a `<rubric_generation>` step inside the prompt — the judge model (Gemini, Claude, etc.) must generate its own rubric at inference time; do NOT pre-write and hardcode a rubric from a different model, which loses the +16.1% IPI anchoring effect (Sage); when fixing, insert a rubric-generation instruction block that the target judge model will execute, not a pre-filled rubric; (b) a small integer rating scale (1–4) with an indicative description per level; (c) a `<reasoning>` or `<evaluation>` field before the verdict field; (d) N>=5 samples with majority vote (not single-pass; N=3 is insufficient for production); (e) confidence-weighted voting when cost matters (matches N=18.6 accuracy at N=10); (f) no debate-style (ChatEval) structure — actively harmful (-158% worst-case per Sage); (g) avoidance of T=0 + seed as a determinism guarantee on Gemini (seed is best-effort; T=0 costs -3pp accuracy per Rating Roulette); (h) multi-model consensus (2-of-3 with Gemini + Claude + GPT) for highest-stakes ranking. Mark N/A for low-stakes filtering and for non-judge prompts.
