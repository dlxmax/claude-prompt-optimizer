...
12. **Judge prompt: self-generated rubric (conditional — highest single-change ROI for judge prompts).** Applies to any prompt whose output is a quality judgment. Does it include a `<rubric_generation>` block instructing the judge model to define scoring criteria before issuing a verdict? The rubric must be generated by the judge model itself at inference time — not pre-written externally and hardcoded (which loses the anchoring effect). When fixing, insert a `<rubric_generation>` instruction block for the judge model to execute, not a filled rubric. Also check: small integer rating scale (1–4) with indicative descriptions, and a `<reasoning>` or `<evaluation>` field before the verdict field. These structural elements together account for the +16.1% IPI gain (Sage, arxiv 2512.16041) and 0.563→0.843 correlation improvement (HuggingFace cookbook). Mark N/A for non-judge prompts.
